<?php
/* @var $this UsuarioController */
/* @var $model Usuario */
/* @var $form CActiveForm */
?>

<div class="form">

<?php $form=$this->beginWidget('CActiveForm', array(
	'id'=>'usuario-form',
	// Please note: When you enable ajax validation, make sure the corresponding
	// controller action is handling ajax validation correctly.
	// There is a call to performAjaxValidation() commented in generated controller code.
	// See class documentation of CActiveForm for details on this.
	'enableAjaxValidation'=>false,
)); ?>

	<p class="note">Fields with <span class="required">*</span> are required.</p>

	<?php echo $form->errorSummary($model); ?>

	<div class="row">
		<?php echo $form->labelEx($model,'PER_CORREL'); ?>
		<?php echo $form->textField($model,'PER_CORREL',array('size'=>10,'maxlength'=>10)); ?>
		<?php echo $form->error($model,'PER_CORREL'); ?>
	</div>

	<div class="row">
		<?php echo $form->labelEx($model,'USU_ESTADO'); ?>
		<?php echo $form->textField($model,'USU_ESTADO',array('size'=>60,'maxlength'=>200)); ?>
		<?php echo $form->error($model,'USU_ESTADO'); ?>
	</div>

	<div class="row">
		<?php echo $form->labelEx($model,'USU_PASSWORD'); ?>
		<?php echo $form->textField($model,'USU_PASSWORD',array('size'=>60,'maxlength'=>200)); ?>
		<?php echo $form->error($model,'USU_PASSWORD'); ?>
	</div>

	<div class="row">
		<?php echo $form->labelEx($model,'USU_CREATE'); ?>
		<?php echo $form->textField($model,'USU_CREATE'); ?>
		<?php echo $form->error($model,'USU_CREATE'); ?>
	</div>

	<div class="row">
		<?php echo $form->labelEx($model,'USU_MODIFIED'); ?>
		<?php echo $form->textField($model,'USU_MODIFIED'); ?>
		<?php echo $form->error($model,'USU_MODIFIED'); ?>
	</div>

	<div class="row buttons">
		<?php echo CHtml::submitButton($model->isNewRecord ? 'Create' : 'Save'); ?>
	</div>

<?php $this->endWidget(); ?>

</div><!-- form -->