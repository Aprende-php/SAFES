...
class Departamento extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('EMP_CORREL, DEP_NOMBRE', 'required'),
			array('EMP_CORREL', 'length', 'max'=>10),
			array('DEP_NOMBRE', 'length', 'max'=>200),
			array('DEP_DESCRIPCION', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('DEP_CORREL, EMP_CORREL, DEP_NOMBRE, DEP_DESCRIPCION', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('DEP_CORREL',$this->DEP_CORREL,true);
		$criteria->compare('EMP_CORREL',$this->EMP_CORREL,true);
		$criteria->compare('DEP_NOMBRE',$this->DEP_NOMBRE,true);
		$criteria->compare('DEP_DESCRIPCION',$this->DEP_DESCRIPCION,true);
